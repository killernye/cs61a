...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    average_lat = mean([restaurant_location(r)[0] for r in cluster])
    average_lon = mean([restaurant_location(r)[1] for r in cluster])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    predictor = lambda x: a + b * feature_fn(x)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    bp = max(feature_fns, key=lambda x: find_predictor(user, reviewed, x)[1])
    return find_predictor(user, reviewed, bp)[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    return {restaurant_name(r): user_rating(user, restaurant_name(r)) if r in reviewed \
             else predictor(r)  for r in restaurants}
    # END Question 9
