...
def roll_dice(num_rolls, dice=six_sided):
    ...
    one_num = 0
    while num_rolls > 0:
        outcome, num_rolls = dice(), num_rolls - 1
        if outcome != 1:
            num_sum = num_sum + outcome
        else:
            one_num += 1
    if one_num > 0:
        return one_num
...
def split_max(n):
    ...
    if n < 10:
        ...
        return max(last, split_max(all_but_last))

def free_bacon(opponent_score):
    """Return the points scored from rolling 0 dice (Free Bacon)."""
    # BEGIN PROBLEM 2
    return split_max(opponent_score) + 1
    # END PROBLEM 2


def is_prime(n):
    """if n is a prime number, return True. Otherwise , False.
    >>> is_prime(1)
    False
    >>> is_prime(3)
    True
    """
    assert type(n) == int and n > 0, 'Your score must be a postive integer.'
    k = 2
    if n == 1:
        return False
    while k < n:
...
def next_prime(n):
    ...
    7
    """
    assert type(n) == int and n > 0, 'n is not a positive integer'
    while True:
        n += 1
        if is_prime(n):
            return n
...
def check_strategy(strategy, goal=GOAL_SCORE):
    ...
    for n in range(0, goal):
        for m in range(0, goal):
            check_strategy_roll(n, m, strategy(n, m))
    # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    for n in range(1, 11):
        ...
        if score > highest_score:
            highest_num, highest_score = n, score
    return highest_num
    # END PROBLEM 8
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_prime(bacon_point):
        bacon_point = next_prime(bacon_point)
    if bacon_point >= margin:
        return 0
    return num_rolls
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_prime(bacon_point):
        bacon_point = next_prime(bacon_point)
    if 2 * (bacon_point + score) == opponent_score:
        return 0
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if 2 * (bacon_point + score) == opponent_score:
        return 0
    points_left_to_win = GOAL_SCORE - score
    margin = min(points_left_to_win, 8)
    dice = select_dice(score, opponent_score, dice_swapped)
    num_rolls = max_scoring_num_rolls(dice, 10)
    return bacon_strategy(score, opponent_score, margin, num_rolls)
    # END PROBLEM 11
