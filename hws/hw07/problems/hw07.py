...
class VendingMachine:
    ...
    def vend(self):
        ...
        spread = abs(self.price - self.received)
        if self.received <  self.price:
            return 'You must deposit ${0} more.'.format(spread)
        else:
            self.received = 0
            self.stock -= 1
            if spread == 0:
                return "Here is your {0}.".format(self.goods)
            return "Here is your {0} and ${1} change.".format(self.goods, spread)

    def deposit(self, amount):
        if self.stock == 0:
            return 'Machine is out of stock. Here is your ${0}.'.format(amount)
        self.received += amount
        return 'Current balance: ${0}'.format(self.received)
