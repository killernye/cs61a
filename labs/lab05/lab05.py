...
def filter(pred, seq):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [elt for elt in seq if pred(elt)]
...
def reduce(combiner, seq):
    ...
    if len(seq) == 1:
        ...
        seq = [combiner(seq[0], seq[1])] + seq[2:]
        return reduce(combiner, seq)
    """
    total = seq[0]
    for elt in seq[1:]:
        total = combiner(total, elt)
    return total
...
def num_songs(t):
    ...
    if is_leaf(t):
        ...
        leaves = [num_songs(b) for b in branches(t)]
        return sum(leaves)
